...
namespace delta
{
    public class AplicacionDelta
    {
                                        ...
                                        out TimeSpan? deltaTiempo, out string mensajeError)
            {
                ...
                nombreArchivoOut = Path.GetDirectoryName(nombreArchivoIn) + Path.DirectorySeparatorChar + 
                                    Path.GetFileNameWithoutExtension(nombreArchivoIn) + "_out" + 
                ...
                {
                    mensajeError = string.Format("Error. Delta de tiempo '{0}' debe estar en formato: [-]hh:mm:ss.fff", 
                                                strDeltaTiempo);
                    return false;                                
                }
